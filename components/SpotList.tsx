...
interface Props {
  ...
  onDrinkFilterChange: (f: DrinkFilter) => void
  onTypeFilterChange: (f: TypeFilter) => void
  onDistrictFilterChange: (f: string) => void
    ...
    <div className="px-8 pb-10">
      <div className="flex flex-wrap items-center gap-3 mb-6 pt-2">
        ...
        <div className="flex items-center gap-1.5">
          <button className={`${btnBase} ${drinkFilter === 'all' ? 'bg-amber-800 text-white' : btnInactive}`} onClick={() => onDrinkFilterChange('all')}>All</button>
          <button className={`${btnBase} ${drinkFilter === 'craft_beer' ? 'bg-[#e07b39] text-white border-transparent' : btnInactive}`} onClick={() => onDrinkFilterChange('craft_beer')}>Beer</button>
          <button className={`${btnBase} ${drinkFilter === 'natural_wine' ? 'bg-[#8b2246] text-white border-transparent' : btnInactive}`} onClick={() => onDrinkFilterChange('natural_wine')}>Wine</button>
        </div>
          ...
          {sorted.map(spot => (
            ...
            >
              ...
              <div className="flex items-center justify-between mt-3">
                <span className="text-amber-800 text-xs font-medium uppercase tracking-wide">{spot.neighborhood}</span>
